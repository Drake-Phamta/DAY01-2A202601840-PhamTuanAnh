...
def streaming_chatbot() -> None:
    ...
    history = []
    while True:
        user_input = input("Your user prompt (type 'quit' to exit): ")
...
def run_assistant(
    ...
    return {"turns": turns, "tokens_used": tokens_used, "total_cost": total_cost, "history": history}
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def shorten(text, max_length=40):
        return text if len(text) <= max_length else text[:max_length-3] + "..."
    row = "| Prompt | GPT-4o Response | Mini Response | GPT-4o Latency | Mini Latency |"
    for results in results:
        row.append(" | " + " | ".join([
            shorten(results["prompt"]),
            shorten(results["gpt4o_answer"]),
            shorten(results["mini_answer"]),
            f"{results['gpt4o_time']:.2f}s",
            f"{results['mini_time']:.2f}s"
        ]) + " |")
    return "\n".join(row)
